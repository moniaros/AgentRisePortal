      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-lg">
        <form onSubmit={handleSubmit(handleFormSubmit)}>
          <div className="p-6 border-b dark:border-gray-700">
            <h2 className="text-xl font-bold">{t('customer.addEventTitle')}</h2>
          </div>
          <div className="p-6 space-y-4">
            <div>
              <label htmlFor="eventType" className="block text-sm font-medium">{t('customer.eventType')}</label>
              <select
                id="eventType"
                {...register("type")}
              ...
              >
                {Object.keys(t('timelineTypes', {})).map(type => (
                   <option key={type} value={type}>{t(`timelineTypes.${type}`)}</option>
                ))}
              </select>
            </div>

            <div>
              <label htmlFor="eventContent" className="block text-sm font-medium">{t('customer.eventDetails')}</label>
              <textarea
                id="eventContent"
                {...register("content", { required: t('validation.required').replace('{fieldName}', t('customer.eventDetails')) })}
                rows={4}
                placeholder={t('customer.eventDetailsPlaceholder')}
            ...
            <div>
                 <label htmlFor="attachment-file" className="block text-sm font-medium">{t('customer.attachments')}</label>
                 <input 
                    type="file"
                    id="attachment-file"
                    multiple
                 ...
                 />
                 <p className="text-xs text-gray-500 mt-1">{t('customer.uploadAttachment')}</p>
            </div>
          </div>
