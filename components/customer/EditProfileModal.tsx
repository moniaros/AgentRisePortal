      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-lg">
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className="p-6 border-b dark:border-gray-700">
            <h2 className="text-xl font-bold">{t('customer.editProfile')}</h2>
          </div>
          <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium">{t('crm.form.firstName')}</label>
                <input type="text" {...register("firstName", { required: t('validation.required').replace('{fieldName}', t('crm.form.firstName')) })} className={`w-full p-2 border rounded dark:bg-gray-700 ${errors.firstName ? 'border-red-500' : 'dark:border-gray-600'}`} />
                {renderError(errors.firstName?.message)}
              </div>
              <div>
                <label className="block text-sm font-medium">{t('crm.form.lastName')}</label>
                <input type="text" {...register("lastName", { required: t('validation.required').replace('{fieldName}', t('crm.form.lastName')) })} className={`w-full p-2 border rounded dark:bg-gray-700 ${errors.lastName ? 'border-red-500' : 'dark:border-gray-600'}`} />
                {renderError(errors.lastName?.message)}
              </div>
            </div>
            <div>
              <label className="block text-sm font-medium">{t('crm.form.email')}</label>
              <input type="email" {...register("email", { required: t('validation.required').replace('{fieldName}', t('crm.form.email')), pattern: { value: /^\S+@\S+$/i, message: t('validation.invalidEmail') } })} className={`w-full p-2 border rounded dark:bg-gray-700 ${errors.email ? 'border-red-500' : 'dark:border-gray-600'}`} />
            ...
            <div>
             ...
             <div>
                <label className="block text-sm font-medium">{t('crm.form.dob')}</label>
