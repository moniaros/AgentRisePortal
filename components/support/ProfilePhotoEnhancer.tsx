...
const ProfilePhotoEnhancer: React.FC = () => {
    ...
    return (
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 flex flex-col h-full">
            ...
            <div className="flex-grow grid grid-cols-1 md:grid-cols-2 gap-8 items-center justify-center py-6">
                {/* Upload / Original */}
                <div className="flex flex-col items-center gap-4">
                    <div 
                        onClick={() => fileInputRef.current?.click()}
                        className="w-40 h-40 rounded-full border-4 border-dashed border-gray-300 dark:border-gray-600 flex items-center justify-center cursor-pointer hover:border-blue-500 transition overflow-hidden bg-gray-50 dark:bg-gray-900 relative group"
                    >
                        {selectedImage ? (
                            <>
                            ...
                            <div className="text-center text-gray-400 p-4">
                                <svg className="w-8 h-8 mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" /></svg>
                                <span className="text-xs font-medium">{t('support.photoTool.upload')}</span>
                            </div>
                        )}
                    </div>
                ...
                {/* Action / Result */}
                <div className="flex flex-col items-center gap-4">
                    <div className={`w-40 h-40 rounded-full border-2 ${enhancedImage ? 'border-green-500 shadow-xl' : 'border-gray-200 dark:border-gray-700'} flex items-center justify-center bg-gray-50 dark:bg-gray-800 text-gray-400 relative overflow-hidden`}>
                        {isProcessing ? (
                            <div className="absolute inset-0 flex items-center justify-center bg-white/80 dark:bg-gray-900/80 z-10">
                                <div className="flex flex-col items-center">
                                    ...
                                    <span className="text-xs font-bold text-blue-600 animate-pulse">AI Magic...</span>
                                </div>
                            </div>
                        ) : enhancedImage ? (
